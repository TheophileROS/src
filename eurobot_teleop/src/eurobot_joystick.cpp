...
using namespace debict::eurobot::teleop;

#define EUROBOT_JOYSTICK_UNSCALED_MAX    32767.0

EurobotJoystick::EurobotJoystick(const std::string & name)
    : rclcpp::Node(name, rclcpp::NodeOptions().use_intra_process_comms(true))
{
    ...
    update_timer_ = create_wall_timer(std::chrono::milliseconds(100), std::bind(&EurobotJoystick::update, this));

    // Reset the device handle
    device_handle_ = -1;
}

EurobotJoystick::~EurobotJoystick()
{
    // Close the joystick
    if (device_handle_ >= 0) {
...
void EurobotJoystick::update()
{
    if (device_handle_ < 0) {
        ...
        if (deadzone_ > 1.0) {
            deadzone_ /= EUROBOT_JOYSTICK_UNSCALED_MAX;
        }
        if (deadzone_ < 0.0) {
            deadzone_ = 0.0;
        }
        scale_ = -1.0f / (1.0f - deadzone_) / EUROBOT_JOYSTICK_UNSCALED_MAX;
        unscaled_deadzone_ = EUROBOT_JOYSTICK_UNSCALED_MAX * deadzone_;
    }
